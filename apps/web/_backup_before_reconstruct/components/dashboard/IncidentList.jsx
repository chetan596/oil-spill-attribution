  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      <div className="flex items-center justify-between px-1 mb-1">
        <span className="text-[11px] font-mono uppercase tracking-[0.08em] text-[#5C5C63]">
        ...
        {items.map((item) => {
          ...
          return (
            <div
              ...
              className={`p-3.5 rounded-lg border transition-all duration-150 cursor-pointer select-none text-left ${
                isSelected
                  ? 'bg-[#141416] border-[#A855F7] shadow-md'
                  : 'bg-[#0A0A0B] border-[#252529] hover:border-[#38383E] hover:bg-[#141416]/50'
              }`}
            >
              ...
              <div className="flex items-start justify-between gap-2">
                  ...
                  <div
                    className={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${
                      isSelected
                        ...
                        : 'bg-[#222224] text-[#5C5C63]'
                    }`}
                  >
                    <Droplet size={13} />
                  </div>
                  <div>
                    <h4 className="text-xs font-medium text-[#F5F5F5] font-sans">
                      {item.title || `Potential Oil Slick #${String(item.id).slice(0, 8)}`}
                    </h4>
                    <span className="text-[10px] font-mono text-[#5C5C63]">
                ...
                <EvidenceBadge
                  classification={item.isRealScene ? 'OBSERVED' : 'OBSERVED'}
                  size="sm"
                />
              </div>

              {/* Coordinates & Physical Dimensions */}
              <div className="grid grid-cols-2 gap-2 mt-3 pt-2.5 border-t border-[#252529]/60 text-[11px]">
                <div className="flex items-center gap-1.5 text-[#71717A] font-mono tabular-nums">
                ...
                <div className="text-right font-mono tabular-nums">
                  <span className="text-[#C8C8CE] font-medium">{area} km²</span>
                  <span className="text-[#5C5C63] text-[10px] ml-1">({conf}%)</span>
                </div>
              </div>

              {/* Bottom Row: Timestamp + Action Link */}
              <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-[#252529]/40 text-[10px] text-[#5C5C63]">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1 text-[#A855F7] hover:text-[#C084FC] font-medium transition-colors"
                >
