    ...
    >
      ...
      <div className="panhead flex items-center justify-between p-4 border-b border-[#252529]">
        <div className="flex items-center gap-2">
          <span className="sw bg-[#a34ff1]" />
          <span className="text-xs font-medium text-[#e8e8ea] font-display tracking-wide">
      ...
      {/* Ledger Entries List */}
      <div className="ledger p-4 space-y-4 max-h-[580px] overflow-y-auto divide-y divide-[#252529]">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="ts font-mono text-[11px] text-[#5C5C63] tabular-nums">
                  {entry.timestamp}
                </span>
                {entry.type && (
              ...
              <h3
                className={`text-xs font-medium mt-1 mb-1 font-display tracking-wide ${
                  entry.accented ? 'text-[#A855F7]' : 'text-[#e8e8ea]'
                }`}
              >
                {entry.title}
              </h3>
              ...
              {/* Summary Description */}
              {entry.description && (
                <p className="text-xs text-[#C8C8CE] leading-relaxed mb-2">
                  {entry.description}
                </p>
              )}
              ...
              {entry.facts && entry.facts.length > 0 && (
                <div className="facts flex items-center gap-3 flex-wrap text-[11px] text-[#5C5C63] font-mono">
                  {entry.facts.map((fact, fIdx) => (
                    <span key={fIdx}>
                      {fact.label}{' '}
                      <b className="text-[#C8C8CE] font-normal tabular-nums">
                        {fact.value}
                      </b>
                    </span>
                  ))}
