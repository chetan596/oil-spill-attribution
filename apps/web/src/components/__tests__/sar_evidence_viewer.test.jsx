...
describe('SAR Evidence & Source Image Viewer Component', () => {
  ...
  it('should support onClose and onShowOnMap callback actions correctly', () => {
    let closed = false;
    let mapShown = false;
    ...
    const element = React.createElement(SarEvidenceViewer, {
      ...
      scene: mockScene1,
      onClose: () => { closed = true; },
      onShowOnMap: () => { mapShown = true; },
    });

    expect(element).toBeDefined();
    expect(element.props.onClose).toBeDefined();
    expect(element.props.onShowOnMap).toBeDefined();

    element.props.onClose();
    element.props.onShowOnMap();

    expect(closed).toBe(true);
    expect(mapShown).toBe(true);
  });

  it('should clearly delineate DEMONSTRATION vs REAL_CDSE scenario types', () => {
    const demoElement = React.createElement(SarEvidenceViewer, {
      isOpen: true,
      spill: mockSpill1,
      scene: { ...mockScene1, scenarioType: 'DEMONSTRATION', isRealScene: false },
    });

    const realElement = React.createElement(SarEvidenceViewer, {
      isOpen: true,
      spill: { ...mockSpill1, scenarioType: 'REAL_CDSE' },
      scene: { ...mockScene1, scenarioType: 'REAL_CDSE', isRealScene: true },
    });

    expect(demoElement.props.scene.scenarioType).toBe('DEMONSTRATION');
    expect(demoElement.props.scene.isRealScene).toBe(false);

    expect(realElement.props.scene.scenarioType).toBe('REAL_CDSE');
    expect(realElement.props.scene.isRealScene).toBe(true);
  });
});
