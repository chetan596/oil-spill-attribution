...
describe('AIS Attribution Mode Components', () => {
  ...
  it('should render AttributionHUD with candidate count, top candidate score, and CPA metrics', async () => {
    ...
    // Verify component renders to string without ReferenceError: useState is not defined
  ...
  it('should render VesselDrawer with Blue Forensic card structure and contained notice', () => {
    const element = React.createElement(VesselDrawer, {
      isOpen: true,
      vessel: mockCandidates[0].vessel,
      onClose: vi.fn(),
    });
    ...
    expect(element.props.vessel.name).toBe('DEMO MARINER ALPHA');
    expect(element.props.vessel.mmsi).toBe('419000123');
  });

  it('should return null when VesselDrawer is closed or has no vessel', () => {
    const closedElement = React.createElement(VesselDrawer, {
      isOpen: false,
      vessel: mockCandidates[0].vessel,
    });
    expect(closedElement.props.isOpen).toBe(false);

    const nullElement = React.createElement(VesselDrawer, {
      isOpen: true,
      vessel: null,
    });
    expect(nullElement.props.vessel).toBeNull();
  });
});
