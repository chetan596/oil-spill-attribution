"""
Execution Script for PART 0.9A — Controlled Multi-Scale Architecture Experiment.
===============================================================================

Objective:
Test whether adding controlled multi-scale spatial context (dilated convolution branches at the bottleneck)
to the dual-pol Sentinel-1 U-Net improves oil-spill segmentation and especially reduces look-alike false positives.

Experimental Variable:
- ARCHITECTURE = MULTI-SCALE CONTEXT U-NET (UNetMultiScaleContext)

Frozen Elements:
- Dataset: 28 Train, 7 Validation, 5 Held-out Test (Locked)
...
from app.models.unet.architecture import UNet, UNetMultiScaleContext
...
def generate_training_curves_svg(history: list, title: str, out_svg_path: str):
    ...
    svg_lines = [
        ...
        f'<text x="{svg_width//2}" y="56" text-anchor="middle" fill="#94a3b8" font-size="12">Training history over {len(history)} epochs | Seed: 42 | UNet Multi-Scale Context</text>',
    ]
...
def generate_threshold_curves_svg(models_threshold_data: Dict[str, Any], metric_name: str, metric_label: str, out_svg_path: str):
    """Generate threshold sweep curve SVG comparing models."""
    ...
    for m_id, th_list in models_threshold_data.items():
        ...
        is_main = m_id == "V09A_MULTISCALE"
        stroke_w = 3.0 if is_main else 2.0
...
def evaluate_multiscale_full_scene_validation(
    model: nn.Module,
    manifest_path: str,
    device: torch.device,
) -> Dict[str, Any]:
...
def main():
    print("=" * 80)
    print("STARTING PART 0.9A — CONTROLLED MULTI-SCALE ARCHITECTURE EXPERIMENT")
    print("=" * 80)

    out_dir = os.path.join(_REPO_ROOT, "ml", "experiments", "results", "v09a_multiscale")
    ...
    v09a_params = count_parameters(v09a_model)
    param_diff = v09a_params - v6_params
    param_pct = (param_diff / v6_params) * 100.0

    print(f"  V6 Baseline Parameters:       {v6_params:,}")
    print(f"  V09A Multi-Scale Parameters:  {v09a_params:,}")
    print(f"  Parameter Difference:         +{param_diff:,} (+{param_pct:.2f}%)")

    if v09a_params > 2.0 * v6_params:
        raise RuntimeError(f"FATAL: Model parameters ({v09a_params}) exceed 2x V6 parameters ({v6_params * 2})!")
    ...
    # Model to device
    v09a_model = v09a_model.to(device)
    optimizer = optim.AdamW(v09a_model.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if ep_dice > best_val_dice:
            ...
            best_epoch = epoch
            best_state_dict = {k: v.cpu().clone() for k, v in v09a_model.state_dict().items()}
            epochs_no_improve = 0
    ...
    checkpoint_rel_path = "ml/model_registry/versions/unet_dual_pol_sar_v09a_multiscale.pth"
    ...
    cpu_model = UNetMultiScaleContext(in_channels=2, num_classes=2, base_channels=16, bilinear=True)
    ...
    if torch.cuda.is_available():
        cuda_model = UNetMultiScaleContext(in_channels=2, num_classes=2, base_channels=16, bilinear=True)
    ...
    eval_model = UNetMultiScaleContext(in_channels=2, num_classes=2, base_channels=16, bilinear=True).to(device)
    eval_model.load_state_dict(best_state_dict)
    eval_model.eval()

    val_eval_results = evaluate_multiscale_full_scene_validation(
        model=eval_model,
    ...
    models_th_data = {
        ...
        "V09A_MULTISCALE": val_eval_results["threshold_aggregates"],
    }

    # Generate SVGs
    generate_training_curves_svg(
        history=history,
        title="Part 0.9A Multi-Scale Context U-Net Training Curves",
    ...
    exp_config = {
        "experiment_id": "PART_0_9A_MULTISCALE_ARCHITECTURE",
        "model_id": "unet-dual-pol-sar-v09a-multiscale",
        "status": "EXPERIMENTAL",
        "architecture": {
            "name": "UNetMultiScaleContext",
            "in_channels": 2,
            "num_classes": 2,
            "base_channels": 16,
            "bilinear": True,
            "context_branches": [
                {"branch": 1, "dilation": 1, "padding": 1, "kernel_size": 3},
                {"branch": 2, "dilation": 2, "padding": 2, "kernel_size": 3},
                {"branch": 3, "dilation": 4, "padding": 4, "kernel_size": 3},
            ],
            "fusion": "Conv2d(1x1) + BatchNorm2d + ReLU",
        },
        "parameter_comparison": {
            "v6_parameters": v6_params,
            "v09a_parameters": v09a_params,
    ...
    print("  [OK] All JSON artifacts created in ml/experiments/results/v09a_multiscale/")

    # Update Registry
    with open(registry_path, "r", encoding="utf-8") as f:
        reg_data = json.load(f)

    # Remove existing entry for v09a if present, then append
    reg_models = [m for m in reg_data.get("models", []) if m.get("model_id") != "unet-dual-pol-sar-v09a-multiscale"]
    reg_models.append({
        "model_id": "unet-dual-pol-sar-v09a-multiscale",
        "architecture": "UNetMultiScaleContext",
        "framework": "PyTorch",
        ...
        "created_at": datetime.now(timezone.utc).isoformat(),
        "description": "Multi-Scale Context U-Net with parallel dilated convolution branches at the bottleneck (dilation=1, 2, 4) for dual-pol SAR.",
        "hyperparameters": {
            "experiment_id": "PART_0_9A_MULTISCALE_ARCHITECTURE",
            "epochs": len(history),
            "best_epoch": best_epoch,
    ...
    print("  [OK] Registry updated: unet-dual-pol-sar-v09a-multiscale registered as status='experimental'")

    print("\n" + "=" * 80)
    print("PART 0.9A MULTI-SCALE EXPERIMENT COMPLETED SUCCESSFULLY!")
    print("=" * 80)
    print(f"Model ID:              unet-dual-pol-sar-v09a-multiscale")
    print(f"Checkpoint:            {checkpoint_rel_path}")
    print(f"SHA-256:               {ckpt_sha}")
    print(f"V6 Params:             {v6_params:,}")
    print(f"V09A Params:           {v09a_params:,} (+{param_diff:,}, +{param_pct:.2f}%)")
