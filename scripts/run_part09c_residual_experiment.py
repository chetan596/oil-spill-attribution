"""
Execution Script for PART 0.9C — Controlled Residual U-Net Architecture Experiment.
===================================================================================

Objective:
Test whether residual convolutional feature blocks in the dual-pol Sentinel-1 U-Net
improve oil-spill segmentation while maintaining acceptable false-positive behavior on look-alike scenes.

Experimental Variable:
- ARCHITECTURE = RESIDUAL U-NET (UNetResidual)
...
def generate_training_curves_svg(history: list, title: str, out_svg_path: str):
    ...
    svg_lines = [
        ...
        f'<text x="{svg_width//2}" y="56" text-anchor="middle" fill="#94a3b8" font-size="12">Training history over {len(history)} epochs | Seed: 42 | Residual U-Net</text>',
    ]
...
def evaluate_residual_full_scene_validation(
    model: nn.Module,
    manifest_path: str,
    device: torch.device,
) -> Dict[str, Any]:
...
def main():
    print("=" * 80)
    print("STARTING PART 0.9C — CONTROLLED RESIDUAL U-NET ARCHITECTURE EXPERIMENT")
    print("=" * 80)

    out_dir = os.path.join(_REPO_ROOT, "ml", "experiments", "results", "v09c_residual")
    ...
    v09c_params = count_parameters(v09c_model)
    param_diff = v09c_params - v6_params
    param_pct = (param_diff / v6_params) * 100.0

    print(f"  V6 Baseline Parameters:       {v6_params:,}")
    print(f"  V09C Residual Parameters:     {v09c_params:,}")
    print(f"  Parameter Difference:         +{param_diff:,} (+{param_pct:.2f}%)")

    if v09c_params > 2.0 * v6_params:
        raise RuntimeError(f"FATAL: Model parameters ({v09c_params}) exceed 2x V6 parameters ({v6_params * 2})!")
    ...
    # Model to device
    v09c_model = v09c_model.to(device)
    optimizer = optim.AdamW(v09c_model.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if ep_dice > best_val_dice:
            ...
            best_epoch = epoch
            best_state_dict = {k: v.cpu().clone() for k, v in v09c_model.state_dict().items()}
            epochs_no_improve = 0
    ...
    checkpoint_rel_path = "ml/model_registry/versions/unet_dual_pol_sar_v09c_residual.pth"
    ...
    val_eval_results = evaluate_residual_full_scene_validation(
        model=eval_model,
    ...
    models_th_data = {
        ...
        "V09C_RESIDUAL": val_eval_results["threshold_aggregates"],
    }

    # Generate SVGs
    generate_training_curves_svg(
        history=history,
        title="Part 0.9C Residual U-Net Training Curves",
    ...
    exp_config = {
        "experiment_id": "PART_0_9C_RESIDUAL_ARCHITECTURE",
        "model_id": "unet-dual-pol-sar-v09c-residual",
        "status": "EXPERIMENTAL",
        "architecture": {
            "name": "UNetResidual",
    ...
    print("  [OK] All JSON artifacts created in ml/experiments/results/v09c_residual/")

    # Update Registry
    with open(registry_path, "r", encoding="utf-8") as f:
        reg_data = json.load(f)

    reg_models = [m for m in reg_data.get("models", []) if m.get("model_id") != "unet-dual-pol-sar-v09c-residual"]
    reg_models.append({
        "model_id": "unet-dual-pol-sar-v09c-residual",
        "architecture": "UNetResidual",
        "framework": "PyTorch",
        ...
        "created_at": datetime.now(timezone.utc).isoformat(),
        "description": "Residual U-Net replacing standard DoubleConv blocks with residual convolutional blocks for dual-pol SAR.",
        "hyperparameters": {
            "experiment_id": "PART_0_9C_RESIDUAL_ARCHITECTURE",
            "epochs": len(history),
            "best_epoch": best_epoch,
    ...
    print("  [OK] Registry updated: unet-dual-pol-sar-v09c-residual registered as status='experimental'")

    print("\n" + "=" * 80)
    print("PART 0.9C RESIDUAL EXPERIMENT COMPLETED SUCCESSFULLY!")
    print("=" * 80)
    print(f"Model ID:              unet-dual-pol-sar-v09c-residual")
    print(f"Checkpoint:            {checkpoint_rel_path}")
    print(f"SHA-256:               {ckpt_sha}")
    print(f"V6 Params:             {v6_params:,}")
    print(f"V09C Params:           {v09c_params:,} (+{param_diff:,}, +{param_pct:.2f}%)")
